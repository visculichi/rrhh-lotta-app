    ...
    <div key={req.id} className="bg-white/80 backdrop-blur-xl p-8 rounded-[48px] border border-slate-50 flex flex-col md:flex-row justify-between items-start md:items-center shadow-lg transition-all hover:shadow-2xl gap-8 animate-in slide-in-from-bottom duration-500">
      <div className="flex-1 w-full">
        <div className="flex items-center gap-6 mb-4">
          <div className={`p-4 rounded-[24px] shadow-sm ${
            req.type === 'cambio de turno' ? 'bg-amber-500 text-white' :
      ...
      <div className="flex flex-col items-end gap-5 min-w-[200px] w-full md:w-auto">
        <div className="flex items-center gap-3">
          ...
          <span className={`px-6 py-2 rounded-2xl text-[10px] font-bold uppercase tracking-widest shadow-sm ${
            req.status === 'aprobado' ? 'bg-emerald-500 text-white' :
...
export const App: React.FC = () => {
  ...
  const fetchSchedule = async (userId: string) => {
    ...
    try {
      ...
      const fullSchedule = DAYS_OF_WEEK.map((_, index) => {
        ...
        return existing || { user_id: userId, day_index: index, start_time: '09:00', end_time: '18:00', is_off: false };
      });
  ...
  const handleSaveSchedule = async () => {
    ...
    try {
      const { error } = await supabase.from('schedules').upsert(
        employeeSchedules.map(s => ({ ...s, user_id: scheduleSelectedUserId })),
  ...
  const handleClockIn = async () => {
    ...
    try {
      ...
      const effectiveRadius = isClockedIn ? officeLocation.radius * 1.5 : officeLocation.radius;
      ...
      <main className="flex-1 overflow-hidden h-full flex flex-col relative">
        ...
        {activeTab === Tab.HOME && (
          <div className="p-8 h-full overflow-y-auto no-scrollbar animate-in fade-in duration-700 space-y-10">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-white p-6 rounded-[24px] border-l-4 border-emerald-400 shadow-sm"><CheckCircle size={16} className="text-emerald-500 mb-2"/><p className="text-2xl font-light text-midnight">{employees.filter(e => e.status === 'presente').length}</p><span className="text-[9px] uppercase font-bold tracking-widest text-slate-400">presentes</span></div>
              <div className="bg-white p-6 rounded-[24px] border-l-4 border-rose-400 shadow-sm"><AlertCircle size={16} className="text-rose-500 mb-2"/><p className="text-2xl font-light text-midnight">{employees.filter(e => e.status === 'ausente').length}</p><span className="text-[9px] uppercase font-bold tracking-widest text-slate-400">ausentes</span></div>
              <div className="bg-white p-6 rounded-[24px] border-l-4 border-sky-400 shadow-sm"><Coffee size={16} className="text-sky-500 mb-2"/><p className="text-2xl font-light text-midnight">{employees.filter(e => { const todayIndex = (currentTime.getDay() + 6) % 7; const sched = allSchedules.find(s => s.user_id === e.id && s.day_index === todayIndex); return sched?.is_off; }).length}</p><span className="text-[9px] uppercase font-bold tracking-widest text-slate-400">francos</span></div>
            ...
            <div className="bg-white/80 backdrop-blur-xl rounded-[40px] border border-white shadow-xl overflow-hidden w-full">
              ...
              <div className="divide-y divide-slate-50">
                {employees.filter(e => e.status === 'presente').map((emp) => {
          ...
          <div className="p-8 h-full overflow-y-auto no-scrollbar animate-in slide-in-from-bottom duration-700">
            <div className="max-w-4xl mx-auto space-y-12">
              ...
              <div className="flex flex-col items-center">
                <ClockInButton onClockIn={handleClockIn} isClockedIn={isClockedIn} isLoading={isClocking} startTime={myCheckInTime} />
              ...
              <div className="bg-white/90 backdrop-blur-xl rounded-[56px] border border-white shadow-2xl overflow-hidden">
                ...
                <div className="divide-y divide-slate-50">
                  {employeeSchedules.map((day, idx) => (
                    <div key={idx} className={`px-12 py-8 flex flex-col md:flex-row items-center justify-between gap-8 transition-colors ${day.is_off ? 'bg-slate-50/50 grayscale-[0.5]' : 'hover:bg-slate-50/30'}`}><div className="flex items-center gap-6 w-full md:w-48"><div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xs font-bold uppercase shadow-sm ${day.is_off ? 'bg-slate-200 text-slate-500' : 'bg-indigo-50 text-indigo-600'}`}>{DAYS_OF_WEEK[idx].substring(0, 2)}</div><div><p className="font-bold text-midnight capitalize">{DAYS_OF_WEEK[idx]}</p><span className={`text-[9px] font-bold uppercase tracking-widest ${day.is_off ? 'text-slate-400' : 'text-emerald-500'}`}>{day.is_off ? 'franco' : 'laboral'}</span></div></div><div className="flex-1 flex flex-wrap items-center justify-center md:justify-end gap-10"><div className="flex items-center gap-4"><div className="space-y-1"><span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest ml-2">ingreso</span><input type="time" disabled={day.is_off || (currentUser?.role !== 'admin' && currentUser?.role !== 'manager')} className={`bg-white border border-slate-200 rounded-xl px-4 py-2 text-sm font-bold text-midnight outline-none transition-all ${day.is_off ? 'opacity-20' : 'focus:ring-2 ring-indigo-100'}`} value={day.start_time} onChange={e => { const newSched = [...employeeSchedules]; newSched[idx].start_time = e.target.value; setEmployeeSchedules(newSched); }}/></div><div className="w-2 h-px bg-slate-300 mt-4"></div><div className="space-y-1"><span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest ml-2">egreso</span><input type="time" disabled={day.is_off || (currentUser?.role !== 'admin' && currentUser?.role !== 'manager')} className={`bg-white border border-slate-200 rounded-xl px-4 py-2 text-sm font-bold text-midnight outline-none transition-all ${day.is_off ? 'opacity-20' : 'focus:ring-2 ring-indigo-100'}`} value={day.end_time} onChange={e => { const newSched = [...employeeSchedules]; newSched[idx].end_time = e.target.value; setEmployeeSchedules(newSched); }}/></div></div><div className="text-center min-w-[60px]"><span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest block mb-1">total hs</span><span className={`text-sm font-mono font-bold ${day.is_off ? 'text-slate-200' : 'text-indigo-600'}`}>{day.is_off ? '0.0' : calculateScheduleHours(day.start_time, day.end_time)}</span></div>{(currentUser?.role === 'admin' || currentUser?.role === 'manager') && <div className="pl-6 border-l border-slate-100 flex items-center gap-3"><span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">franco:</span><button onClick={() => { const newSched = [...employeeSchedules]; newSched[idx].is_off = !newSched[idx].is_off; setEmployeeSchedules(newSched); }} className={`w-14 h-7 rounded-full relative transition-all duration-300 ${day.is_off ? 'bg-indigo-500' : 'bg-slate-200'}`}><div className={`absolute top-1.5 w-4 h-4 bg-white rounded-full shadow-sm transition-all duration-300 ${day.is_off ? 'right-1.5' : 'left-1.5'}`}></div></button></div>}</div></div>
                  ))}
                </div>
              </div>
            </div>
        ...
        {activeTab === Tab.DOCS && (
          <div className="p-10 h-full overflow-y-auto no-scrollbar animate-in slide-in-from-bottom duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-20">{docs.map(doc => (<div key={doc.id} className="bg-white/80 backdrop-blur-xl p-8 rounded-[48px] border border-slate-50 flex flex-col shadow-lg transition-all hover:shadow-2xl animate-in slide-in-from-bottom duration-500 group relative"><div className="absolute top-6 right-8 flex gap-2"><div className={`flex items-center gap-1.5 px-3 py-1 rounded-full border shadow-sm ${doc.visibility === 'private' ? 'bg-rose-50 text-rose-600 border-rose-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>{doc.visibility === 'private' ? <LockIcon size={10}/> : <Globe size={10}/>}<span className="text-[8px] font-bold uppercase tracking-widest">{doc.visibility === 'private' ? 'privado' : 'todos'}</span></div></div><div className="flex items-center gap-4 mb-6"><div className="p-4 bg-midnight/5 text-midnight rounded-[20px] shadow-inner"><Files size={24}/></div><div className="flex-1 min-w-0 pr-12"><h3 className="font-bold text-midnight text-lg truncate lowercase">{doc.title}</h3><p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{doc.category} <span className="mx-2 opacity-30">|</span> {doc.date}</p></div></div>{doc.signatureData ? (<div className="mb-6 p-4 bg-emerald-50 border-2 border-dashed border-emerald-200 rounded-2xl flex items-center gap-3 shadow-inner"><Stamp size={24} className="text-emerald-500 shrink-0"/><div className="flex flex-col overflow-hidden"><span className="text-[8px] font-bold uppercase tracking-widest text-emerald-600">CERTIFICADO DE FIRMA ELECTRÓNICA</span><span className="text-[9px] font-medium text-emerald-700 italic leading-tight break-all">{doc.signatureData}</span></div></div>) : doc.requiresSignature ? (<div className="mb-6 p-4 bg-amber-50 border border-amber-100 rounded-2xl flex items-center justify-between"><div className="flex items-center gap-3"><Stamp size={20} className="text-amber-500"/><span className="text-[9px] font-bold uppercase tracking-widest text-amber-600">pendiente de firma</span></div><button onClick={() => setSigningDocId(doc.id)} className="px-4 py-2 bg-amber-500 text-white rounded-xl text-[8px] font-bold uppercase tracking-widest shadow-sm hover:scale-105 active:scale-95 transition-all">firmar ahora</button></div>) : null}{doc.allSignatures && doc.allSignatures.length > 0 && (<div className="mt-6 border-t border-slate-100 pt-4"><p className="text-[9px] font-bold uppercase tracking-widest text-slate-400 mb-3 flex items-center gap-2"><CheckCircle size={12} className="text-emerald-500"/>firmas registradas ({doc.allSignatures.length})</p><div className="space-y-2 max-h-40 overflow-y-auto no-scrollbar bg-slate-50/50 p-2 rounded-xl">{doc.allSignatures.map((sig, i) => (<div key={i} className="flex items-center gap-2 text-[10px]"><div className="w-1.5 h-1.5 rounded-full bg-emerald-400"></div><span className="font-bold text-slate-600 capitalize">{sig.profiles?.full_name || 'usuario'}</span><span className="text-slate-400 ml-auto font-mono">{new Date(sig.signed_at).toLocaleDateString('es-AR')}</span></div>))}</div></div>)}<p className="text-sm text-slate-500 line-clamp-2 mb-8 flex-1 lowercase mt-4">{doc.description || 'sin descripción adicional'}</p><div className="flex items-center justify-between mt-auto bg-slate-50/50 p-4 rounded-[28px] border border-slate-100"><div className="flex items-center gap-2"><div className="w-6 h-6 rounded-full bg-white shadow-sm flex items-center justify-center text-[10px] font-bold text-slate-400 uppercase">{getInitials(doc.userName || '')}</div><span className="text-[10px] font-bold text-slate-400 lowercase truncate max-w-[80px]">{doc.userName}</span></div><div className="flex items-center gap-1"><button onClick={() => handleDownload(doc.fileUrl, doc.title)} title="descargar" className="p-3 bg-white text-indigo-600 rounded-2xl hover:bg-indigo-600 hover:text-white shadow-sm transition-all active:scale-95"><Download size={18}/></button><button onClick={() => handlePreview(doc.fileUrl)} title="ver en pestaña" className="p-3 bg-white text-emerald-600 rounded-2xl hover:bg-emerald-600 hover:text-white shadow-sm transition-all active:scale-95"><ExternalLink size={18}/></button>{(currentUser?.role === 'admin' || currentUser?.role === 'manager') && (<button onClick={() => setConfirmDelete({ isOpen: true, id: doc.id, name: doc.title, type: 'document' })} className="p-3 bg-white text-rose-500 rounded-2xl hover:bg-rose-500 hover:text-white shadow-sm transition-all opacity-0 group-hover:opacity-100"><Trash2 size={18}/></button>)}</div></div></div>))} {docs.length === 0 && <div className="col-span-full py-20 text-center"><FolderOpen size={48} className="mx-auto text-slate-200 mb-4"/><p className="text-slate-400 font-medium lowercase">no hay documentos cargados aún o no tienes permisos para verlos</p></div>}</div>
          </div>
        )}
